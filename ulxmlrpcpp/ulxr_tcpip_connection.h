...
class  TcpIpConnection : public Connection
{
 ...
 /**
   ...
   */
   ...
   static void setNonblock(int fd, bool aSet);
   
   //@nothrow
   //@return if non-blocking connecton succeeds the function return true and anErrorMsg stays intact
   //                                  otherwise the function return false and anErrorMsg is appended with the error message
   bool connectNonBlocking(int sock, const struct sockaddr *name, socklen_t namelen, unsigned int aConnectTimeoutSec, std::string& anErrorMsg, bool anIsIpv6);
 ...
 private:
   ...
   bool isIpv4;
   bool isIpv6;
   const size_t theTcpConnectionTimeout;
};
